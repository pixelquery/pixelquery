...
Examples:
  ...
  pixelquery recovery repair ./warehouse   Repair warehouse issues
    ...
    info_parser = subparsers.add_parser(
        "info",
        help="Show warehouse information"
    )
    info_parser.add_argument("warehouse", help="Warehouse path")
    info_parser.add_argument(
        "--snapshots",
        action="store_true",
        help="Show snapshot history (Time Travel)"
    )

    # Migrate command
    migrate_parser = subparsers.add_parser(
        "migrate",
        help="Migrate Arrow storage to Iceberg"
    )
    ...
    recovery_parser = subparsers.add_parser(
        "recovery",
        help="Diagnose and repair warehouse issues"
    )
    ...
    diagnose_parser = recovery_subparsers.add_parser(
        "diagnose",
        help="Diagnose warehouse issues"
    )

    repair_parser = recovery_subparsers.add_parser(
        "repair",
        help="Repair warehouse issues"
    )
    repair_parser.add_argument(
        "--cleanup-orphaned",
        action="store_true",
        help="Remove unreferenced data files (CAUTION)"
    )

    verify_parser = recovery_subparsers.add_parser(
        "verify",
        help="Verify warehouse integrity"
    )
