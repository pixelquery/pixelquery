...
from pixelquery._internal.storage.arrow_chunk import ArrowChunkWriter, ArrowChunkReader
...
class TestArrowChunkReader:
    ...
    def test_read_chunk_basic(self, reader, sample_chunk):
        ...
        data, metadata = reader.read_chunk(sample_chunk)
...
class TestArrowChunkAppend:
    ...
    def test_append_multiple_times(self, temp_dir):
        ...
        for i in range(n_obs):
            data = {
                'time': [datetime(2024, 1, i+1, tzinfo=timezone.utc)],
...
class TestArrowChunkRoundtrip:
    ...
    def test_roundtrip_multiple_observations(self, temp_dir):
        ...
        n_obs = 5
        original_data = {
            'time': [datetime(2024, 1, i+1, tzinfo=timezone.utc) for i in range(n_obs)],
            'pixels': [np.random.randint(0, 10000, size=100, dtype=np.uint16) for _ in range(n_obs)],
